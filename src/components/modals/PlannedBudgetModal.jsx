...
const PlannedBudgetModal = ({ onClose }) => {
  const { darkMode, plannedBudget, setPlannedBudget, allCategories, showAlert } = useApp();
  const [budgets, setBudgets] = useState({});

  useEffect(() => {
    setBudgets(plannedBudget || {});
  }, [plannedBudget]);
  ...
  const handleSave = async () => {
    try {
      await api.updatePlannedBudget(budgets);
      setPlannedBudget(budgets);
      showAlert('Succès', 'Budget prévisionnel enregistré', 'success');
      onClose();
    } catch (error) {
      ...
      <div className={`w-full max-w-lg ${darkMode ? 'bg-slate-800' : 'bg-white'} rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden`} onClick={e => e.stopPropagation()}>
        <div className={`p-4 border-b ${darkMode ? 'border-slate-700' : 'border-gray-200'}`}>
          <div className="flex items-center justify-between">
            <h3 className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>📅 Budget Prévisionnel</h3>
            <button onClick={onClose} className={`p-2 rounded-lg ${darkMode ? 'hover:bg-slate-700' : 'hover:bg-gray-100'}`}>✕</button>
          </div>
          <p className={`text-sm mt-1 ${darkMode ? 'text-slate-400' : 'text-gray-500'}`}>
            Planifiez vos dépenses pour le mois prochain
          </p>
        </div>
        ...
        <div className={`p-4 border-t ${darkMode ? 'border-slate-700' : 'border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <span className={`font-medium ${darkMode ? 'text-slate-300' : 'text-gray-700'}`}>Total prévu</span>
...
export default PlannedBudgetModal;